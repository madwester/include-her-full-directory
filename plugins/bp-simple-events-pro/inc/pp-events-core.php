...
class PP_Events_Component extends BP_Component {
	...
	function includes( $includes = array() ) {
		...
		if( ! is_admin() ) {
			...
			if( bp_is_active('groups') && PP_GROUPS )
				$includes[] = 'inc/pp-events-groups.php';
		...
		$includes[] = '/inc/pp-events-attending.php';

		if ( bp_is_active( 'notifications' ) )
			$includes[] = 'inc/pp-events-notifications.php';
	...
	function setup_globals( $args = array() ) {
		...
		$globals = array(
			...
			'notification_callback' => 'pp_events_format_notifications',
	...
	function setup_nav( $main_nav = array(), $sub_nav = array() ) {
		...
		bp_core_new_subnav_item( array(
			'name'              =>  __( 'Attending', 'bp-simple-events' ),
			'slug'              => 'attending',
			'parent_url'        => trailingslashit( $user_domain . 'events' ),
			'parent_slug'       => 'events',
			'screen_function'   => 'pp_events_profile_attending',
			'position'          => 40,
			'item_css_id'       => 'member-events-attending'
			//'user_has_access'   => $user_has_access
			)
		);		
	...
	function setup_admin_bar( $wp_admin_nav = array() ) {
		...
		if ( is_user_logged_in() ) {
			...
			$wp_admin_nav[] = array(
				...
				'title'  => __( 'Attending', 'bp-simple-events' ),
				'href'   => trailingslashit( $item_link ) . 'attending'
			);
