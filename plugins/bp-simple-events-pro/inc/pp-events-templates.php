...
//  load events-loop or events-map template from theme or plugin
	...
	if( ! bp_is_user() ) {
		...
		if( isset( $wp_query->query['pagename'] ) ) {
			...
			if ( $wp_query->query['pagename'] == 'events' ) {
				...
				if ( isset( $_GET['e'] ) && $_GET['e'] == 'emap' ) {
					
					
					$theme_template = locate_template( 'events-map.php' );
		
					if( file_exists( $theme_template ) )
					   $template = $theme_template;
					else
					   $template = PP_EVENTS_DIR . '/templates/events-map.php';					
